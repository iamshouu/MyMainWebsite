...
import { SOCIAL_LINKS, TRADING_STATS, SKILL_DATA, PROJECTS_DATA, MEDIA_CONFIG, PERFORMANCE_DATA, MONTHLY_PERFORMANCE_DATA, TRADING_DETAILED_STATS, CERTIFICATES_DATA } from './constants';
...
import { RadarChart, PolarGrid, PolarAngleAxis, Radar as RechartsRadar, ResponsiveContainer, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ReferenceLine, BarChart, Bar, Cell, LabelList } from 'recharts';
...
const App: React.FC = () => {
  const [viewMode, setViewMode] = useState<'main' | 'projects' | 'performance'>('main');
    ...
    <div className="bg-black text-white h-[100dvh] w-full relative overflow-hidden font-sans selection:bg-white/20 selection:text-white">
      ...
      >
            ...
            <Sparkles>
              <div className="relative group mb-4 md:mb-8 logo-container py-4">
                ...
                <div className="mt-2 md:mt-4 flex items-center justify-between w-full px-2">
                   ...
                   <div className="px-3 md:px-4 text-[7px] md:text-[9px] font-mono text-white/80 tracking-[0.3em] md:tracking-[0.6em] uppercase">Private access</div>
                   <div className="h-[1px] bg-white/20 flex-1" />
                </div>
              </div>
            </Sparkles>
            
            <p className="text-white/80 text-[9px] md:text-xs font-mono mb-6 md:mb-10 tracking-[0.5em] md:tracking-[0.8em] uppercase flex items-center gap-3 md:gap-4">
        ...
        <Section id="experience">
           <div className="w-full max-w-6xl mx-auto z-10 grid md:grid-cols-2 gap-8 md:gap-16 items-center">
             <div className="animate-fade-in-up">
                ...
                <div className="space-y-3 md:space-y-5">
                  {TRADING_STATS.map((stat, idx) => (
                    <div key={idx} className="bg-white/5 backdrop-blur-md border border-white/5 p-4 md:p-8 rounded-xl md:rounded-[1.5rem] hover:border-white/20 hover:bg-white/10 transition-all duration-500 group">
                       ...
                       <p className="text-[11px] md:text-[13px] text-white/60 mt-2 md:mt-3 font-medium leading-relaxed">{stat.description}</p>
                    </div>
                  ))}
                </div>
             </div>
             <div className="h-[300px] md:h-[450px] bg-black/40 rounded-xl md:rounded-[2rem] border border-white/10 p-4 md:p-8 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-white/20 to-transparent" />
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="48%" cy="50%" outerRadius="74%" data={SKILL_DATA}>
                    <PolarGrid stroke="rgba(255,255,255,0.05)" />
                    <PolarAngleAxis dataKey="subject" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 8, fontWeight: 500, letterSpacing: '0.05em' }} />
                    <RechartsRadar name="Skills" dataKey="A" stroke="#ffffff" fill="#ffffff" fillOpacity={0.1} />
                  </RadarChart>
                </ResponsiveContainer>
             </div>
           </div>
                  ...
                  <div key={idx} className={`group relative p-6 md:p-8 bg-white/5 border rounded-xl md:rounded-[2.5rem] transition-all duration-500 overflow-hidden ${plan.isHighlight ? 'border-white/40 bg-white/10 shadow-[0_0_40px_rgba(255,255,255,0.05)]' : 'border-white/5 hover:bg-white/10 hover:border-white/20'}`}>
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      ...
                      <div className="flex items-center gap-2 mb-3 md:mb-4">
                        ...
                        {plan.status === "Soon" && (
                          <span className="px-1.5 py-0.5 bg-white/10 border border-white/20 rounded text-[7px] font-mono text-white/50 animate-pulse uppercase">SOON</span>
                        )}
                      </div>
        ...
        <Section id="connect">
           <div className="text-center w-full max-w-5xl flex flex-col h-full justify-center">
              <h2 className="text-5xl md:text-[12rem] font-black mb-10 md:mb-20 opacity-[0.03] text-white uppercase tracking-tighter select-none pointer-events-none">CONTACT</h2>
              ...
              <p className="mt-10 md:mt-32 text-[8px] md:text-[10px] font-mono text-white/70 uppercase tracking-[0.4em] md:tracking-[0.7em]">
                shou.trade // secure connection
              </p>
           </div>
        </Section>
      </main>
      ...
      {viewMode === 'projects' && (
        <div className="fixed inset-0 z-[200] bg-black p-4 md:p-24 overflow-y-auto">

...
const APPLE_HELLO_ENGLISH_ANIMATION_DURATION_SECONDS = 3.5;
  ...
  return (
    ...
    >
      ...
      <motion.path
        ...
        transition={{ duration: calc(0.3), ease: 'easeInOut', opacity: { duration: 0.15 } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(0.7), ease: 'easeInOut', delay: calc(0.4), opacity: { duration: calc(0.35), delay: calc(0.4) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(0.5), ease: 'easeOut', delay: calc(1), opacity: { duration: calc(0.25), delay: calc(1) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(0.3), ease: 'easeOut', delay: calc(1.5), opacity: { duration: calc(0.15), delay: calc(1.5) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(0.9), ease: 'easeOut', delay: calc(1.8), opacity: { duration: calc(0.45), delay: calc(1.8) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(1.1), ease: 'easeInOut', delay: calc(2.6), opacity: { duration: calc(0.55), delay: calc(2.6) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(1), ease: 'easeInOut', delay: calc(3.6), opacity: { duration: calc(0.5), delay: calc(3.6) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(0.8), ease: 'easeOut', delay: calc(4.6), opacity: { duration: calc(0.4), delay: calc(4.6) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(1.5), ease: 'easeOut', delay: calc(5.4), opacity: { duration: calc(0.75), delay: calc(5.4) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={animateProps}
        transition={{ duration: calc(0.8), ease: 'easeInOut', delay: calc(7), opacity: { duration: calc(0.4), delay: calc(7) } }}
...
function AppleHelloEnglishEffect({
  ...
  useEffect(() => {
    ...
    const startTimer = window.setTimeout(() => {
      startFrame = window.requestAnimationFrame(() => setIsDrawing(true));
    }, 120);
  ...
  useEffect(() => {
    if (!isDrawing || !onAnimationComplete) {
      return undefined;
    }

    const completionTimer = window.setTimeout(
      onAnimationComplete,
      APPLE_HELLO_ENGLISH_ANIMATION_DURATION_SECONDS * speed * 1000,
    );

    return () => window.clearTimeout(completionTimer);
  }, [isDrawing, onAnimationComplete, speed]);
  ...
  return (
    ...
    >
      ...
      <motion.path
        ...
        style={{ strokeLinecap: 'round' }}
        initial={initialProps}
        animate={isDrawing ? animateProps : initialProps}
        transition={{ duration: calc(0.8), ease: 'easeInOut', opacity: { duration: calc(0.4) } }}
      />
      <motion.path
        ...
        initial={initialProps}
        animate={isDrawing ? animateProps : initialProps}
        transition={{ duration: calc(2.8), ease: 'easeInOut', delay: calc(0.7), opacity: { duration: calc(0.7), delay: calc(0.7) } }}
      />
    </motion.svg>
  );
}

type DrawnGreetingEffectProps = Omit<Props, 'text'> & {
  text: string;
  viewBoxWidth: number;
  fontSize?: number;
  fontFamily?: string;
...
function DrawnGreetingEffect({
  ...
  fontFamily = 'cursive',
  ...props
}: DrawnGreetingEffectProps) {
  const calc = (x: number) => x * speed;
  const characters = Array.from(text);
  const isCjk = characters.some((character) => /[\u3400-\u9fff\uf900-\ufaff]/u.test(character));
  const characterSpacing = Math.min(
    (viewBoxWidth * 0.78) / Math.max(characters.length - 1, 1),
    fontSize * (isCjk ? 0.95 : 0.68),
  );
  const firstCharacterX = viewBoxWidth / 2 - (characterSpacing * (characters.length - 1)) / 2;
  ...
  return (
    ...
    >
      ...
      {characters.map((character, index) => {
        const delay = calc(index * 0.46);

        return (
          <motion.text
            key={`${character}-${index}`}
            x={firstCharacterX + characterSpacing * index}
            y="148"
            textAnchor="middle"
            ...
            style={{ strokeDasharray: dashLength }}
            initial={{ strokeDashoffset: dashLength, opacity: 0 }}
            animate={{ strokeDashoffset: 0, opacity: 1 }}
            transition={{
              strokeDashoffset: { duration: calc(0.84), delay, ease: 'easeInOut' },
              opacity: { duration: calc(0.16), delay },
            }}
...
export { AppleHelloEnglishEffect, AppleHelloVietnameseEffect, DrawnGreetingEffect };
